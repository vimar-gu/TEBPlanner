...
CWorld::CWorld() {
    for (int i = 0; i < Config::FieldConfig::ROBOT_NUMBER; i++) {
        Robot robot(Config::FieldConfig::ORIGIN_X + 200, Config::FieldConfig::ORIGIN_Y + i * 50);
        robotVec.push_back(robot);
    }
    for (int i = 0; i < Config::FieldConfig::OBSTACLE_NUMBER; i++) {
        Obstacle obs(Config::FieldConfig::ORIGIN_X, Config::FieldConfig::ORIGIN_Y + i * 50);
...
void CWorld::addObstacle() {
    int x = rand() % 10;
    int y = rand() % 10;
    Obstacle obs(Config::FieldConfig::ORIGIN_X + x * 50, Config::FieldConfig::ORIGIN_Y + y * 50);
    obsVec.push_back(obs);
}
